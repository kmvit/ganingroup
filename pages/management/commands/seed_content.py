...
OBJECTS = [
    ('Реконструкция аэропорта «Минеральные Воды»', 'Минеральные Воды', 'бетон',
     '24 000 м³ бетона (2024) — непрерывные круглосуточные поставки, стратегический объект региона'),
...
TEAM = [
    ('Михаил Ганин', 'собственник группы'),
    ('уточняется', 'операционный директор СД-2'),
    ('Клиновой', 'директор UPEX'),
    ('Дронкин', 'начальник карьера'),
]

# Заказчик: на сайте только кнопка звонка в отдел персонала — список вакансий не публикуем.
VACANCIES = []

TIMELINE = [
    ('1962', 'Основание производства',
     'Запуск производственной площадки на Кавказских Минеральных Водах.'),
    ('2010', 'Модернизация: смесительные узлы Teka',
     'Немецкое смесительное оборудование — рост качества и объёмов товарного бетона.'),
    ('2011', 'Формовочные линии ZENITH',
     'Немецкие линии ЖБИ — до 293 видов изделий.'),
    ('2015', 'Собственный карьер',
     'Открытие карьера между станицами Зольская и Марьинская — своё сырьё.'),
    ('2019', 'Запуск UPEX',
...
REVIEW = ('«Для нас важнее всего был <em>ритм поставок</em> на пике сезона. График монолита '
          ...
          'Николай Алексеевич Галка', 'директор ООО СЗ «Неруд Зольское»')
...
CONCRETE_GRADES = [
    ...
    ('М500', 'В40 W6-8', 8785, 'высокомарочный'),
...
# Документы берём из единого списка content/docs_seed.py (файлы — в media/docs/)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        s = SiteSettings.get()
        s.founded_year = 1962
        s.phone_main = '+7 928 821-72-11, +7 928 821-72-02'
        s.phone_dispatch = ''
        s.phone_upex = ''
        s.email_sales = 'nsale@sd2kmv.ru'
        s.email_main = 'nsale@sd2kmv.ru'
